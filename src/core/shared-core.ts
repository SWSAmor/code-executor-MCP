/**
 * SharedCore — transport-agnostic core for the Code Executor MCP server.
 *
 * Owns the shared, long-lived components (downstream MCP client pool, security
 * validator, connection pool, rate limiter, health-check server) and the
 * free-function tool registration that a per-connection `McpServer` binds to.
 *
 * Extracted from the original `CodeExecutorServer` class so a future HTTP
 * transport can build these shared pieces ONCE and register the same tools on a
 * per-session `McpServer`, while the stdio entrypoint (src/index.ts) stays a
 * thin wrapper. Behavior over stdio is unchanged — this is a pure extraction.
 */
...
/**
 * Transport-agnostic shared core.
 *
 * Built once per process; the tool set (see {@link registerTools}) is bound to
 * whatever `McpServer` a transport creates.
 */
export class SharedCore {
   ...
   *
   * Must run before {@link registerTools} (registration reads `denoAvailable`).
   */
   ...
   *
   * The caller decides when to terminate the process — this method never calls
   * process.exit so it is reusable across transports.
   *
   ...
   * Backstop for the process 'exit' handler (see the stdio entrypoint).
   */
 ...
 */
export function handleToolError(error: unknown, errorType: ErrorType) {
...
export async function checkRateLimit(
  ...
  if (!result.allowed) {
    ...
    return handleToolError(error, ErrorType.VALIDATION);
  }
    ...
    "timeoutMs": 60000
      ...
      const typescriptToolHandler: Parameters<McpServer['registerTool']>[2] = async (args: any, extra: RequestHandlerExtra<any, any>) => {
        try {
          ...
          // Validate security
          core.securityValidator.validateAllowlist(input.allowedTools);
          await core.securityValidator.validatePermissions(input.permissions);
          ...
          const codeValidation = core.securityValidator.validateCode(input.code, skipPatternCheck);
          ...
          const result = await core.connectionPool.execute(async () => {
            return await executeTypescriptInSandbox(
              ...
              core.mcpClientPool,
              server.server  // Pass underlying Server instance with request() method for MCP sampling
            );
          });

          // Audit log
          await core.securityValidator.auditLog(
            {
              executor: 'typescript',
              allowedTools: input.allowedTools,
          ...
          return handleToolError(error, ErrorType.EXECUTION);
        }
      };
...
Returns:
  {
    ...
    "uptime": number
      ...
      async (args: any, extra: RequestHandlerExtra<any, any>) => {
        try {
          const tools = core.mcpClientPool.listAllTools();
          const poolStats = core.connectionPool.getStats();

          const health = {
            healthy: true,
            auditLog: {
              enabled: core.securityValidator.isAuditLogEnabled(),
            },
            mcpClients: {
